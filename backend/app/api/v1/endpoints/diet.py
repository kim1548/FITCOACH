    ...
    try:
        ...
        if results[0].boxes and len(results[0].boxes) > 0:
            for i, box in enumerate(results[0].boxes):
                ...
                crop_targets.append((cropped, crop_filename))
        else:
            # 그릇을 전혀 못 찾으면 원본 전체 이미지 사용
            crop_filename = f"{uuid.uuid4()}_full.jpg"
            crop_path = os.path.join(UPLOAD_DIR, crop_filename)
            original_img.save(crop_path, "JPEG")
            crop_targets.append((original_img, crop_filename))

        # 2. EfficientNet 분류기 예측 및 리스트 적재
        for cropped_img, filename in crop_targets:
            print(f"====================================")
            print(filename)
            ...
            with torch.no_grad():
                ...
                # 🛠️ 여기에 디버깅용 프린트 추가!                
                foodname = FOOD_NUTRITION_DB.get(idx.item())
                print(f"🔮 모델이 뱉은 순수 숫자 인덱스(idx): {foodname}")
                print(f"📊 모델의 확신도(Confidence): {round(current_conf * 100, 2)}%")
                # --------------------
            ...
            if f_info:
                ...
                # ⭐️ 이름이 같더라도 개별 그릇이므로 씹지 않고 전부 결과 리스트에 누적합니다.
                final_detected_items.append({
                    "food_name": food_name,
                    "confidence": round(current_conf * 100, 2), # 확신도 % 표시
                    "calories": f_info["kcal"],
                    "carbs": f_info["carbs"],
                    "protein": f_info["protein"],
                    "fat": f_info["fat"],
                    "image_url": f"/static/uploads/food/{filename}"
                })
